 ...
 * @prop {Boolean} defaultValue - (Optional) Define true/false status. Default value is false.
 */
export const Toggle = () => (
  <>
    <ToggleSwitch onChange={(status) => console.log('Toggle status is ' + status)} />
    <ToggleSwitch defaultValue={true} onChange={(status) => console.log('Toggle status is ' + status)} />
